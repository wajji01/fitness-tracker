...
// ── Reusable Components ────────────────────────────────────────────────────────
function SectionCard({ title, description, icon, children, dark }) {
  return (
    <div className={`rounded-2xl border overflow-hidden transition-colors ${dark ? "bg-gray-900 border-gray-700" : "bg-white border-gray-100"}`}>
      <div className={`px-6 py-5 border-b flex items-center gap-3 ${dark ? "border-gray-700" : "border-gray-50"}`}>
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${dark ? "bg-violet-900 text-violet-400" : "bg-violet-50 text-violet-600"}`}>
          {icon}
        </div>
        <div>
          <h2 className={`text-sm font-bold ${dark ? "text-gray-100" : "text-gray-900"}`}>{title}</h2>
          {description && <p className={`text-xs mt-0.5 ${dark ? "text-gray-400" : "text-gray-400"}`}>{description}</p>}
        </div>
      </div>
...
function InputField({ label, name, type = "text", value, onChange, disabled, placeholder, required, dark }) {
  return (
    <div>
      ...
      <input
        ...
        className={`w-full px-3.5 py-2.5 text-sm rounded-xl border outline-none transition-colors ${
          disabled
            ? dark ? "bg-gray-800 border-gray-700 text-gray-500 cursor-not-allowed" : "bg-gray-50 border-gray-100 text-gray-400 cursor-not-allowed"
            : dark ? "bg-gray-800 border-gray-700 text-gray-100 focus:border-violet-500 focus:ring-2 focus:ring-violet-900" : "border-gray-200 bg-white focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
        }`}
      />
    </div>
  );
...
export default function Settings() {
  ...
  const [avatarPreview, setAvatarPreview] = useState(""); // local blob/base64 preview
  ...
  const [prefs, setPrefs]           = useState({ darkMode: dark, weeklyGoal: 5 });
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        const savedAvatar = user.avatar || user.profilePicture || "";
        setProfile({ name: user.name || "", email: user.email || "", avatar: savedAvatar });
        setAvatarPreview(savedAvatar); // show existing pic
        const savedDark = user.preferences?.darkMode ?? (localStorage.getItem("darkMode") === "true");
        setPrefs({ darkMode: savedDark, weeklyGoal: user.preferences?.weeklyGoal ?? 5 });
        setDark(savedDark);
  ...
  const handleProfileSave = async () => {
    ...
    setProfileMsg({ type: "", text: "" });
    try {
      let avatarToSend = profile.avatar;

      // If new file selected — send as base64 or use FormData
      if (avatarFile) {
        ...
        const newAvatar = updated.avatar || updated.profilePicture || avatarPreview;
        setProfile(p => ({ ...p, name: updated.name || p.name, avatar: newAvatar }));
        setAvatarPreview(newAvatar);
        setAvatarFile(null);
      } else {
        // No new file — just update name
        const { data } = await axios.put(`${BASE}/users/me`, { name: profile.name, avatar: avatarToSend }, authHeaders());
        const updated = data.user || data;
  ...
  return (
    <div className={`max-w-2xl mx-auto space-y-6 transition-colors ${dark ? "text-gray-100" : ""}`}>

      {/* Header */}
      <div>
        <h2 className={`text-xl font-extrabold ${dark ? "text-gray-100" : "text-gray-900"}`}>Settings</h2>
        <p className="text-sm text-gray-400 mt-0.5">Manage your account and preferences</p>
      </div>

      {/* ── Profile Section ── */}
      <SectionCard dark={dark} title="Profile Information" description="Update your name and profile picture"
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative flex-shrink-0 group">
              {avatarPreview ? (
                <img
                  src={avatarPreview}
                  alt="Profile"
                  className="w-20 h-20 rounded-2xl object-cover border-2 border-violet-200"
                />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-sm font-semibold ${dark ? "text-gray-100" : "text-gray-800"}`}>{profile.name || "Your Name"}</p>
              ...
              <div className="mt-3 flex items-center gap-2 flex-wrap">
                ...
                {avatarPreview && (
                  <button
                    onClick={() => { setAvatarPreview(""); setAvatarFile(null); setProfile(p => ({ ...p, avatar: "" })); }}
                    className="text-xs font-semibold text-red-500 hover:text-red-600 px-3 py-1.5 rounded-lg border border-red-100 bg-red-50 hover:bg-red-100 transition-colors"
                  >
                    Remove
                  </button>
                )}
                {avatarFile && (
                  <span className="text-xs text-emerald-600 font-medium">✓ {avatarFile.name}</span>
                )}
              </div>
              <p className="text-xs text-gray-400 mt-1.5">JPG, PNG or WEBP · Max 5MB · Hover photo to change</p>
              {/* Hidden file input */}
              <input
                ref={fileRef}
                type="file"
          ...
          <div className="grid gap-4">
            <InputField dark={dark} label="Full Name" name="name" value={profile.name} required
              placeholder="Enter your full name"
              onChange={e => setProfile(p => ({ ...p, name: e.target.value }))}
            />
            <InputField dark={dark} label="Email Address" name="email" type="email" value={profile.email} disabled />
          </div>
      ...
      {/* ── Password Section ── */}
      <SectionCard dark={dark} title="Change Password" description="Keep your account secure with a strong password"
            ...
            <div key={key}>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-3.5 py-2.5 pr-11 text-sm rounded-xl border outline-none transition-colors ${
                    dark
                      ? "bg-gray-800 border-gray-700 text-gray-100 focus:border-violet-500 focus:ring-2 focus:ring-violet-900"
              ...
              {key === "newPass" && passwords.newPass && (
                <div className="mt-2 flex items-center gap-1.5">
                  {[1, 2, 3].map(i => {
                    const strength = passwords.newPass.length >= 10 ? 3 : passwords.newPass.length >= 6 ? 2 : 1;
                    return <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= strength ? strength === 1 ? "bg-red-400" : strength === 2 ? "bg-amber-400" : "bg-emerald-400" : "bg-gray-200"}`} />;
                  })}
      ...
      {/* ── Preferences Section ── */}
      <SectionCard dark={dark} title="Preferences" description="Customize your fitness tracking experience"
      ...
      >
        <div className="space-y-5">
          ...
          {/* Dark Mode Toggle */}
          <div className={`flex items-center justify-between p-4 rounded-xl border ${dark ? "bg-gray-800 border-gray-700" : "bg-gray-50 border-gray-100"}`}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-gray-800 flex items-center justify-center text-yellow-400 flex-shrink-0">
                {prefs.darkMode ? (
              ...
              <div>
                <p className={`text-sm font-semibold ${dark ? "text-gray-100" : "text-gray-800"}`}>Dark Mode</p>
                <p className="text-xs text-gray-400">{prefs.darkMode ? "Currently dark theme" : "Currently light theme"}</p>
              </div>
            </div>
            <button
              onClick={() => setPrefs(p => ({ ...p, darkMode: !p.darkMode }))}
              className={`relative w-12 h-6 rounded-full transition-colors duration-200 ${prefs.darkMode ? "bg-violet-600" : "bg-gray-300"}`}
            >
              <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-transform duration-200 ${prefs.darkMode ? "translate-x-6" : "translate-x-0"}`} />
            </button>
          </div>

          {/* Weekly Goal */}
          <div className={`p-4 rounded-xl border ${dark ? "bg-gray-800 border-gray-700" : "bg-gray-50 border-gray-100"}`}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className={`text-sm font-semibold ${dark ? "text-gray-100" : "text-gray-800"}`}>Weekly Workout Goal</p>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setPrefs(p => ({ ...p, weeklyGoal: Math.max(1, p.weeklyGoal - 1) }))}
                className={`w-9 h-9 rounded-xl border flex items-center justify-center font-bold transition-colors ${dark ? "border-gray-600 bg-gray-700 hover:bg-gray-600 text-gray-200" : "border-gray-200 bg-white hover:bg-gray-50 text-gray-600"}`}>−</button>
              <div className="flex-1 text-center">
                <span className="text-2xl font-extrabold text-violet-600">{prefs.weeklyGoal}</span>
                <span className={`text-sm ml-1.5 ${dark ? "text-gray-400" : "text-gray-400"}`}>workouts / week</span>
              </div>
            ...
            <div className="flex gap-1.5 mt-3">
              {Array.from({ length: 14 }, (_, i) => (
                <button key={i} onClick={() => setPrefs(p => ({ ...p, weeklyGoal: i + 1 }))}
                  className={`flex-1 h-1.5 rounded-full transition-colors ${i < prefs.weeklyGoal ? "bg-violet-500" : dark ? "bg-gray-600" : "bg-gray-200"}`} />
              ))}
            </div>
          </div>
      ...
      {/* ── Account / Logout ── */}
      <SectionCard dark={dark} title="Account" description="Manage your session"
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <p className={`text-sm font-semibold ${dark ? "text-gray-100" : "text-gray-800"}`}>Sign Out</p>
